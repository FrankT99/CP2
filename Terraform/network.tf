...
resource "azurerm_network_interface_security_group_association" "sec_group_ssh-vm-cp2" {
  ...
  network_security_group_id = azurerm_network_security_group.sec_group_ssh.id
}

resource "azurerm_network_interface" "nic" {
  name                = "${var.virtual_machine_vm_name}-nic"
  location            = azurerm_resource_group.rg.location
  resource_group_name = azurerm_resource_group.rg.name

  ip_configuration {
    name                          = "${var.virtual_machine_vm_name}-ip_config"
